...
x_train, y_train, x_test, y_test, scaler = load_data("data_cleaned/RELIANCE.csv")
...
# actual_prices and predicted_prices from your model
rmse = np.sqrt(mean_squared_error(actual_prices, predicted_prices))
mae = mean_absolute_error(actual_prices, predicted_prices)
...
plt.title("Reliance Stock Prediction with Sentiment")
plt.xlabel("Days")
plt.ylabel("Price")
plt.legend()
plt.show()
